  ...
  const handleSelect = async (method: string) => {
    ...
    if (method === "cash") {
      ...
      if (isTemporaryOrder) {
        ...
        const createResponse = await fetch("https://mobile-login-be.onrender.com/api/orders", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        try {
          ...
          const updateResponse = await fetch(`https://mobile-login-be.onrender.com/api/orders/${orderInfo.id}`, {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
            },
          ...
          if (updateResponse.ok) {
            ...
            if (updatedOrder.tableId) {
              try {
                ...
                const ordersResponse = await fetch("https://mobile-login-be.onrender.com/api/orders");
                ...
                if (otherActiveOrders.length === 0) {
                  ...
                  const tableUpdateResponse = await fetch(
                    `https://mobile-login-be.onrender.com/api/tables/${updatedOrder.tableId}/status`,
                    {
                      method: "PUT",
                      headers: {
                        "Content-Type": "application/json",
    ...
    if (isTemporaryOrder) {
      ...
      try {
        ...
        const statusResponse = await fetch(
          `https://mobile-login-be.onrender.com/api/orders/${orderInfo.id}/status`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
