...
export function OrderReport() {
  ...
  const { data: orders = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch(`https://4beac38c-34b4-47be-8df2-4a7d6f34c6b5-00-yd16h0ayqss7.pike.replit.dev/api/orders/date-range/${startDate}/${endDate}`);
        if (!response.ok) {
  ...
  const { data: transactions = [] } = useQuery({
    queryKey: ["https://4beac38c-34b4-47be-8df2-4a7d6f34c6b5-00-yd16h0ayqss7.pike.replit.dev/api/transactions", startDate, endDate],
    queryFn: async () => {
      try {
        const response = await fetch(`https://4beac38c-34b4-47be-8df2-4a7d6f34c6b5-00-yd16h0ayqss7.pike.replit.dev/api/transactions/${startDate}/${endDate}`);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
