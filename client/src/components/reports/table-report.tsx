...
export function TableReport() {
  ...
  const { data: orders = [], isLoading: ordersLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("https://api-pos-login.edpos.vn/api/orders");
        if (!response.ok) {
  ...
  const { data: invoices = [], isLoading: invoicesLoading } = useQuery({
    queryKey: ["https://api-pos-login.edpos.vn/api/invoices"],
    queryFn: async () => {
      try {
        const response = await fetch("https://api-pos-login.edpos.vn/api/invoices");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const { data: transactions = [], isLoading: transactionsLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await fetch("https://api-pos-login.edpos.vn/api/transactions");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const { data: transactionItems = [], isLoading: transactionItemsLoading } =
    useQuery({
      queryKey: ["https://api-pos-login.edpos.vn/api/transaction-items"],
      queryFn: async () => {
        try {
          const response = await fetch("https://api-pos-login.edpos.vn/api/transaction-items");
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
          }
          const data = await response.json();
