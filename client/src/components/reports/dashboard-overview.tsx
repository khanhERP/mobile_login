...
export function DashboardOverview() {
  ...
  const { data: dateRangeOrders } = useQuery({
    queryKey: ["https://api-pos-login.edpos.vn/api/orders/date-range", dateRange.start, dateRange.end],
    queryFn: async () => {
      try {
        console.log(`Dashboard - Date Range Query:`, {
          startDate: dateRange.start,
          endDate: dateRange.end,
          apiUrl: `https://api-pos-login.edpos.vn/api/orders/date-range/${dateRange.start}/${dateRange.end}`,
        });

        const response = await fetch(
          `https://api-pos-login.edpos.vn/api/orders/date-range/${dateRange.start}/${dateRange.end}`,
        );
        if (!response.ok) {
